...
public class NIOClient100 extends AbstractConfig {


    private SocketChannel socketChannel;
    private Selector selector;

    public NIOClient100(int port) throws IOException {
        SocketChannel socketChannel = SocketChannel.open();
        socketChannel.configureBlocking(false);
        selector = Selector.open();
        // 注册连接服务端socket动作
        socketChannel.register(selector, SelectionKey.OP_CONNECT);
        InetSocketAddress SERVER_ADDRESS = new InetSocketAddress(IOConfig.HOST, IOConfig.PORT);
    ...
    public static void main(String[] args) throws IOException {
        for (int i = 0; i < 1; i++) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    NIOClient100 client = null;
                    try {
                        client = new NIOClient100(IOConfig.PORT);
                        client.execute();
